...
### 시작 화면 & 데이터셋 선택
접속하면 먼저 **시작 화면**이 뜹니다: ① 내 이름 ② 구글시트 URL(기본값 프리필) ③ **평가할 데이터셋 버튼** 선택 → "뷰어 열기".
- 뷰어 안에서 좌상단 **⟳ 데이터셋** 버튼으로 다시 선택 화면으로 돌아갈 수 있습니다.
- 시작 화면의 "다른 CSV 파일 직접 올리기"로 아직 배포 안 된 파일을 즉석에서 열 수도 있습니다(내 브라우저에서만).

**새 월 데이터셋 추가법** (예: 7월):
1. `gzip -c 7월_잘쓰는유저.csv > data/2026-07-good.csv.gz` (못쓰는유저도 동일)
2. `index.html`의 `DATASETS` 배열에 한 줄 추가:
   ```js
   { id: '2026-07', label: '2026년 7월', sub: '7월 생성자 · 잘쓰는+못쓰는',
     good: 'data/2026-07-good.csv.gz', bad: 'data/2026-07-bad.csv.gz' },
   ```
3. `git push` → 시작 화면에 버튼이 자동으로 추가됩니다.
